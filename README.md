...
3. Configure environment variables:
   ...
   RESEND_API_KEY=<your Resend key — leave unset to keep mail off>
   MAIL_FROM=noreply@formamorph.ai
...
### Authentication
- `POST /api/auth/register` - Register a new user. `email` is optional; a taken one is refused with `EMAIL_TAKEN`
...
An address is optional and never blocks play: the account works the moment it is made. Giving one mails a
link to `SITE_URL/verify-email?token=…`, and opening it stamps the address verified. The link works once
and expires after a day; a newly issued one retires whatever the account held before, so the newest mail
is always the one that works. The token is never stored — the row keeps a hash of it — so a leak of the
database hands out no working links.
...
Most failures carry only `error`, a sentence to show the user. These also carry a `code`, because the
