 ...
 *
 * Four steps: build the placements table, rename the broadcast column to the results vocabulary, lift
 * whatever winner a contest already recorded into first place with its snapshots, and rebuild `events`
 * without the columns that held the old answer. A contest with a winner counts as announced — the pick
 * *was* the announcement — so the stamp is backfilled from the row's own `updated_at` rather than
 * invented as now.
 *
 ...
 * rebuild rather than `DROP COLUMN`, which SQLite refuses for a column named in a foreign key — while a
 * *rename* of such a column it performs happily, which is why the broadcast id needs no rebuild.
 *
...
const addEventPlacements = (database = db) => {
  ...
  let columns = database.prepare('PRAGMA table_info(events)').all().map((column) => column.name);

  // The broadcast column keeps its value and its foreign key and only changes name — the message it
  // points at is the podium announcement now, not a winner. Renamed rather than rebuilt away: SQLite
  // refuses to *drop* a column named in a foreign key but renames one happily, carrying the constraint
  // and its ON DELETE with it. Its own step, because a database migrated before this rename still has
  // the old name with none of the winner columns left to trigger the rebuild below.
  if (columns.includes('winner_message_id') && !columns.includes('results_message_id')) {
    ...
    console.log('Renamed winner_message_id to results_message_id');
    columns = database.prepare('PRAGMA table_info(events)').all().map((column) => column.name);
    changed = true;
  }
